...
-   checks sensitivity based on "power-scaling"
    -   (sensitivity to varying strength of prior or likelihood)
...
- Default proper priors for $\beta_{\text{Intercept}}$ and $\sigma$

- Try a weakly informative prior $\beta_{\text{grouptrt1}} \sim \text{normal}(0, 2.5)$ and perform a prior-predictive check
...
Trying a wider prior: $\beta_i \sim \text{normal}(0, 50)$
...
```{r}
knitr::kable(powerscale_sensitivity(npk_fit2)$sensitivity, digits = 2)
